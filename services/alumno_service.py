...
class AlumnoService:
    ...
    def crear(self, datos: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            Alumno_id = self.db.execute_stored_procedure("sp_CrearAlumno", params)
    ...
    def obtener(self, alumno_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Ejecutar SP
            resultados = self.db.execute_stored_procedure(
                "sp_ObtenerAlumno", (alumno_id,)
            )
    ...
    def inscribirAlumnoEnCarrera(self, alumno_id: int, carrera_id: int) -> bool:
        ...
        try:
            ...
            self.db.execute_stored_procedure(
                "sp_InscribirAlumnoCarrera", (alumno_id, carrera_id)
            )
            return True
    ...
    def inscribirAlumnoEnAsignatura(self, alumno_id: int, asignatura_id: int) -> bool:
        ...
        try:
            # Ejecutar SP
            self.db.execute_stored_procedure(
                "sp_InscribirAlumnoAsignatura", (alumno_id, asignatura_id)
            )
            return True
    ...
    def registrarNota(self, alumno_id: int, asignatura_id: int, nota: float) -> bool:
        ...
        try:
            # Ejecutar SP
            self.db.execute_stored_procedure(
                "sp_RegistrarNota", (alumno_id, asignatura_id, nota)
            )
            return True
    ...
    def obtenerHistorialAcademico(
        ...
        try:
            # Ejecutar SP
            resultados = self.db.execute_stored_procedure(
                "sp_ObtenerHistorialAcademico", (alumno_id,)
            )
    ...
    def verificarCorrelativas(self, alumno_id: int, asignatura_id: int) -> bool:
        ...
        try:
            # Ejecutar SP
            resultados = self.db.execute_stored_procedure(
                "sp_VerificarCorrelativas", (alumno_id, asignatura_id)
            )

            if resultados and len(resultados) > 0:
                return (
                    resultados[0][0] == 1
                )  # Retorna True si cumple con las correlativas
